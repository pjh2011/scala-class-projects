...
object TweetLength {
  ...
  def tweetRemainingCharsCount(tweetText: Signal[String]): Signal[Int] = {
    val tweetString: String = tweetText()
    val remaining_chars: Int = MaxTweetLength - tweetLength(tweetString)
    Signal(remaining_chars)
  }

  def colorForRemainingCharsCount(remainingCharsCount: Signal[Int]): Signal[String] = {
    val chars_remaining = remainingCharsCount()

    val tweet_color = {
      if (chars_remaining >= 15) "green"
      else if (chars_remaining >= 0) "orange"
      else "red"
    }
